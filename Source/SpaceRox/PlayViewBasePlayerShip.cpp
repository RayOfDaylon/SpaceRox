...
void UPlayViewBase::ProcessPlayerShipCollision(float Mass, const FVector2D* PositionOther, const FVector2D* InertiaOther)
{
	check(PlayerShip);

	if(!PlayerShip->ProcessCollision(Mass, PositionOther, InertiaOther))
	{
		KillPlayerShip();
	}
}
